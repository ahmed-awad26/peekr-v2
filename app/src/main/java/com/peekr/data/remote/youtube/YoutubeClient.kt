...
class YoutubeClient @Inject constructor(
    ...
    suspend fun syncChannels(): Result<Int> = withContext(Dispatchers.IO) {
        try {
            ...
            val account = accountDao.getAccountByPlatform("youtube")
            if (account == null || !account.isConnected) {
                logger.warning("يوتيوب مش مربوط", "youtube")
                return@withContext Result.failure(Exception("يوتيوب مش مربوط"))
            }

            // جلب الـ channels من الـ extraData
            val channelUrls = account.extraData?.split(",") ?: emptyList()
            var totalNewPosts = 0

            channelUrls.forEach { channelUrl ->
                try {
                    val channelId = resolveChannelId(channelUrl.trim(), apiKey)
                    if (channelId != null) {
                        val newPosts = fetchChannelVideos(channelId, apiKey)
                        totalNewPosts += newPosts
                    }
                } catch (e: Exception) {
                    logger.error("فشل جلب فيديوهات القناة: $channelUrl", "youtube", e)
                }
            }
